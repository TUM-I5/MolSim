...
void XMLInputReader::readInput(ProgramParameters &programParameters, const char *filename)
{
    ...
    try
    {
        ...
        for (simulation_t::file_name_const_iterator i(xml->file_name().begin()); i != xml->file_name().end(); i++)
        {
            ...
            programParameters.readFromFile(path.c_str());
        }

        for (simulation_t::cuboid_const_iterator i(xml->file_name().begin()); i != xml->file_name().end(); i++)
        {
        ...
        for (simulation_t::sphere_const_iterator i(xml->file_name().begin()); i != xml->file_name().end(); i++)
        {
