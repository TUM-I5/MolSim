...
CuboidSpawner::CuboidSpawner(std::array<double, 3>& lower_left_corner, std::array<size_t, 3>& grid_dimensions, double grid_spacing, double mass, std::array<double, 3>& initial_velocity, int type, double avg_velocity) : lower_left_corner(lower_left_corner), grid_dimensions(grid_dimensions), grid_spacing(grid_spacing), mass(mass), type(type), initial_velocity(initial_velocity), avg_velocity(avg_velocity) {}
...
void CuboidSpawner::spawnParticles(ParticleContainer& particle_container) {
    for (size_t i = 0; i < grid_dimensions[0]; i++) {
        for (size_t j = 0; j < grid_dimensions[1]; j++) {
            for (size_t k = 0; k < grid_dimensions[2]; k++) {
