...
/**
 * using a lambda function interface for the calcualtion of the force between
 * two Particles, takes in two Particles and returns the vector3 of forces
 * acting between the two given Particles
 * simplified:
 * forceCalculation refers to such functions "std::array<double,3> func(const
 * Particle&,const Particle&)" uses constant references because forceCalculation
 * mustn't change the Particles
 */
...
int main(int argc, char *argsv[]) {
  auto msg =
      ...
      "\n"
      "Returns:\n"
      "                  several .vtu files that can be used for visualisation "
      "in Paraview\n";
    ...
    switch (opt) {
      ...
      case 'e':
        try {
          ...
          std::cerr << "Invalid argument for the endtime" << e.what()
                    << std::endl;
        } catch (const std::out_of_range &e) {
...
void calculateF(forceCalculation forcecalc) {
  static std::pair<Particle *, Particle *> pair =
...
void calculateX() {
  ...
  while (p != nullptr) {
    ...
    for (int i = 0; i < 3; i++) {
      p->setX(i,
              old_x[i] + delta_t * v[i] + delta_t * delta_t * f[i] / 2.0 / m);
    }
